...
from cipher.ecc import ECCCipher
 #Thêm vào đâu file này

app = Flask(__name__)

# RSA cipher algorithm
rsa_cipher = RSACipher()
rsa_cipher.generate_keys()  # Chỉ gọi nếu muốn tạo sẵn khi khởi động
...
def rsa_encrypt():
    data = request.json
    message = data.get('plain_text')  # ✅ Sửa chỗ này cho đúng
...
def ecc_sign_message():
    ...
    signature = ecc_cipher.encrypt(message, private_key)
    signature_hex = signature.hex()
    return jsonify({'signature': signature_hex})
...
def ecc_verify_signature():
    ...
    signature = bytes.fromhex(signature_hex)
    is_verified = ecc_cipher.decrypt(message, signature, public_key)
